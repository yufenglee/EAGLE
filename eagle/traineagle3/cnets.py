...
class Model(nn.Module):
    ...
    def scandata(self, datapath, tokenizerpath):
        """
        Scan training data to build draft model vocabulary.
        This analyzes token frequencies to determine the most common tokens.
        
        Args:
            datapath: Path to training data file
        ...
        if not os.path.exists("cache.pt"):
            ...
            def preprocess_for_vocab(examples):
                """Preprocess examples to extract input_ids and loss_mask for vocab building."""
                new_examples = {
                    "input_ids": [],
                    "loss_mask": []
                }
                for i in range(len(examples['id'])):
                    ...
                    # Skip first message if not from user
                    ...
                    try:
                        for j, sentence in enumerate(conversations[start_idx:]):
                            ...
                            assert role == CONV_ROLES[j % 2], f"Unexpected role at {i}"
